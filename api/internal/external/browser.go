...
type Browser interface {
	...
	// Click 通过索引或坐标点击元素
	Click(ctx context.Context, sessionID string, index *int, coordinateX, coordinateY *float64) (*model.ToolResult, error)

	// Input 在输入框中输入文本
	Input(ctx context.Context, sessionID, text string, pressEnter bool, index *int, coordinateX, coordinateY *float64) (*model.ToolResult, error)

	// MoveMouse 移动鼠标到指定坐标
	MoveMouse(ctx context.Context, sessionID string, coordinateX, coordinateY float64) (*model.ToolResult, error)
	...
	// ScrollDown 向下滚动浏览器
	ScrollDown(ctx context.Context, sessionID string, toDown *bool) (*model.ToolResult, error)
