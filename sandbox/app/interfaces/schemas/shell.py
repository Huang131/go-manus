...
class ShellExecuteRequest(BaseModel):
    ...
    command: str = Field(..., min_length=1, max_length=64 * 1024, description="要执行的 Shell 命令")


class ShellReadRequest(BaseModel):
    """查看Shell执行内容请求结构体"""
    session_id: str = Field(..., min_length=1, max_length=128, description="目标 Shell 会话的唯一标识符")
...
class ShellWaitRequest(BaseModel):
    """等待Shell命令执行请求结构体"""
    session_id: str = Field(..., min_length=1, max_length=128, description="目标 Shell 会话的唯一标识符")
    seconds: Optional[int] = Field(default=None, ge=1, le=24 * 60 * 60, description="等待时间, 单位为秒")


class ShellWriteRequest(BaseModel):
    """写入数据到子进程请求结构体"""
    session_id: str = Field(..., min_length=1, max_length=128, description="目标 Shell 会话的唯一标识符")
    input_text: str = Field(..., max_length=64 * 1024, description="需要写入的内容文本")
