 ...
 * API 配置
 */
// API 配置（与后端路由前缀对齐：所有接口挂在 /api 下，无版本号）
// - NEXT_PUBLIC_API_BASE_URL = /api          （推荐：nginx 反代模式，浏览器走 80/443）
// - NEXT_PUBLIC_API_BASE_URL = http://localhost:8080/api  （无 nginx 直连 API 容器，依赖 CORS）
// - 默认值：/api，便于配合 nginx 部署；无 nginx 时改为绝对地址即可。
const API_CONFIG = {
