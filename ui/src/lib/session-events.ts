...
export function eventsToTimeline(events: SSEEventData[]): TimelineItem[] {
  ...
  for (const ev of events) {
    switch (ev.type) {
      ...
      case "step": {
        // 后端 step 事件格式：{ step: { id, description, status, ... }, status: "started"|"completed"|... }
        // 提取嵌套的 step 对象并把外层 status 合并到 step.status
        const rawStep = ev.data as { step?: StepEvent; status?: string };
        const step: StepEvent = {
          ...(rawStep.step ?? (ev.data as StepEvent)),
          status: (rawStep.step?.status ?? (ev.data as StepEvent).status) as StepEvent["status"],
        };
      ...
      case "tool_called": {
        ...
        const tool: ToolEvent = {
          ...
          content,
          // display 单独承载仅 UI 可见的数据（大体积产物在后端落存储，这里只有文件引用），与 content 互不干扰
...
export function getLatestPlanFromEvents(events: SSEEventData[]): PlanStep[] {
  ...
  for (let i = events.length - 1; i >= 0; i--) {
    const ev = events[i];
    if (ev.type === "plan") {
      // 后端 plan 事件格式：{ plan: { steps: [...], title, ... }, status }
      const rawPlan = ev.data as { plan?: PlanEvent } & PlanEvent;
